...
const AttendanceList = () => {
  ...
  const totalRecords = allAttendance?.data?.records?.length || 0
  const totalPresent = allAttendance?.data?.records?.reduce((sum, record) => sum + record.presentEntries, 0) || 0
  const totalAbsent = allAttendance?.data?.records?.reduce((sum, record) => sum + record.absentEntries, 0) || 0
  const totalLate = allAttendance?.data?.records?.reduce((sum, record) => sum + record.lateEntries, 0) || 0
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm">Manage and track employee attendance records</p>
          </div>
        </div>
        <div className="mt-4 md:mt-0 flex items-center gap-2 bg-white px-4 py-2 rounded-lg shadow-sm">
          <Calendar className="h-4 w-4 text-indigo-500" />
          <span className="text-sm font-medium text-slate-700">{totalRecords} Records Found</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white rounded-xl shadow-sm p-6 border-l-4 border-green-500 flex items-center justify-between">
          <div>
            ...
            <h4 className="text-3xl font-bold text-green-600">{totalPresent}</h4>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border-l-4 border-amber-500 flex items-center justify-between">
          <div>
            ...
            <h4 className="text-3xl font-bold text-amber-600">{totalLate}</h4>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border-l-4 border-red-500 flex items-center justify-between">
          <div>
            ...
            <h4 className="text-3xl font-bold text-red-600">{totalAbsent}</h4>
          </div>
                  ...
                  >
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <span className="text-sm font-medium text-slate-800">{attendance.date}</span>
                      </div>
                    </td>
